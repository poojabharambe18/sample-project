...
@Service
public class AccountClosedProcessData extends CommonBase {

	static Logger LOGGER = LoggerFactory.getLogger(AccountClosedProcessData.class);

	private static final String PROC_GET_ACCT_DATA = "PACK_ACCT_CLOSE.PROC_GET_ACCT_DATA";
	...
	public String getAccountClosedProcessData(String input) {
		...
		try {
			...
			loggerImpl.info(LOGGER, "preparing the request data and calling the API", "IN:getAccountClosedProcessData");
			loggerImpl.generateProfiler("getAccountClosedProcessData");
			...
			loggerImpl.startProfiler("Calling getAccountClosedProcessData API response data.");
			...
			ls_response = getProcData.getCursorData(connection,PROC_GET_ACCT_DATA, ls_giCompCd, ls_giBranchCd, ls_giCompCd,
					ls_branchCd, ls_acctType, ls_acctCd, workingDt, ls_userNm, ls_machineNm, ls_userLevel, ls_screenRef,
					ls_lang);
			...
			return getExceptionMSg(exception, LOGGER, loggerImpl, "IN:getAccountClosedProcessData", "(ENP551)");
		}finally {
